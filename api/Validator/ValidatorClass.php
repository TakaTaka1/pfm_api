...
{
	public static function validation ($input_control,$input_params) {
		$result = [];
		foreach($input_control as $attr_name => $info) {
			if(array_key_exists($attr_name, $input_params)){ // 属性チェック
				...
				foreach($rules as $rule){					
					...
					switch($rule_name) {
						case "empty":
							if(self::is_empty($input_value)) {
								$result[$attr_name][] = "{$info['name']}は必須です。";
							}
							break;
						case "is_num":
							if(!self::is_valid_price($input_value)) {
								$result[$attr_name][] = "{$info['name']}は半角数値で入力してください。";
							}
							break;
						case "max":
							if(!empty($rule_setting) && self::is_max($input_value,$rule_setting)) {
								$result[$attr_name][] = "{$info['name']}は{$rule_setting}以下で入力してください。";
							}
							break;
						case "email":
							if(self::is_valid_email($input_value)){
